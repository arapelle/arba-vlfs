...
{
    static const std::regex virtual_root_regex(R"([a-zA-Z][\w.-]{1,7})", std::regex::ECMAScript | std::regex::optimize);
...
{
    if (auto vroot_iter = virtual_root_map_.find(vroot); vroot_iter != virtual_root_map_.end()) [[likely]]
        return true;
...
bool virtual_filesystem::is_virtual_path_(path_string_view path, std::size_t& pos)
{
    ...
    {
        std::size_t path_view_length =
            std::min<std::size_t>(path.length() - virtual_root_marker.length(), strn::string64::max_length());
...
{
template <class T>
    requires requires(T iter, std::size_t n) { std::string_view(iter, n); }
